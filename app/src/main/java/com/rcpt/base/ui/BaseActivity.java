...
import com.rcpt.utils.RefreshHandler;
...
public abstract class BaseActivity<D extends ViewDataBinding, V extends BaseView, P extends BasePresenter<V>> extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (isUseDefaultTitle) {
            initTitle();
            if (mTitleCompatLayout == null) {

                try {
                    throw new NullPointerException("请检查您的布局文件中是否引入了自定义的标题栏");
                } catch (Exception e) {
                    Log.e("err", e.getMessage());
                }
            }
        }
        StatusBarCompat.setStatusBarColor(this, mTitleCompatLayout, true);
        setupTitle();
        initViews();
    ...
    protected void setTitleVisible(boolean isVisible) {
        if (isVisible) {
            mTitleCompatLayout.setVisibility(View.VISIBLE);
        } else {
            mTitleCompatLayout.setVisibility(View.GONE);
        }
    }
